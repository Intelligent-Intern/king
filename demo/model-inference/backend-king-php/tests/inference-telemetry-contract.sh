...
if [[ ! -x "${LLAMA_CPP_HOME}/llama-server" ]]; then
  echo "[inference-telemetry-contract] SKIP: llama.cpp runtime not installed." >&2
  exit 0
fi
if [[ ! -f "${GGUF_FIXTURE_PATH}" ]]; then
  echo "[inference-telemetry-contract] SKIP: GGUF fixture not installed." >&2
  exit 0
fi
...
if "${PHP_BIN}" -m | grep -Eiq '^king$'; then
  ...
  echo "[inference-telemetry-contract] SKIP: King extension not available." >&2
  exit 0
fi
php_args+=("-d" "king.security_allow_config_override=1")

if ! "${PHP_BIN}" "${php_args[@]}" -r 'exit(extension_loaded("king") ? 0 : 1);' 2>/dev/null; then
  echo "[inference-telemetry-contract] FAIL: King extension present but failed to load." >&2
  exit 1
fi

export LLAMA_CPP_HOME
export MODEL_INFERENCE_GGUF_FIXTURE_PATH="${GGUF_FIXTURE_PATH}"
"${PHP_BIN}" "${php_args[@]}" "${SCRIPT_DIR}/inference-telemetry-contract.php"
