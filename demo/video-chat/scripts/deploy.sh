...
# shellcheck source=demo/video-chat/scripts/lib/deploy-remote-status.sh
source "${SCRIPT_DIR}/lib/deploy-remote-status.sh"
...
ensure_hcloud_dns_records_if_configured() {
  ...
  [[ -n "${VIDEOCHAT_DEPLOY_HCLOUD_TOKEN:-}" && -n "${DEPLOY_PUBLIC_IP}" ]] || {
    log "Hetzner DNS refresh skipped; set VIDEOCHAT_DEPLOY_HCLOUD_TOKEN or create DNS records manually."
    return 0
  }
  require_cmd curl
  require_cmd jq
  ...
  export HCLOUD_TOKEN HCLOUD_API_BASE
...
for cert_domain in "\${EXTRA_CERT_DOMAINS[@]}"; do
  cert_domain="\${cert_domain//[[:space:]]/}"
  if [ -n "\${cert_domain}" ]; then
    CERTBOT_DOMAINS+=(-d "\${cert_domain}")
  fi
done
...
VIDEOCHAT_STUN_URIS=stun:\${TURN_DOMAIN}:3478
...
VITE_VIDEOCHAT_ENABLE_WLVC=true
...
VIDEOCHAT_PRODUCTION_SOURCEMAPS=hidden
...
VIDEOCHAT_SPUTNIK_MAX_COUNT=25
...
VITE_VIDEOCHAT_ICE_SERVERS=stun:\${TURN_DOMAIN}:3478
ENVEOF
...
set_env_value VIDEOCHAT_STUN_URIS "stun:\${TURN_DOMAIN}:3478"
...
set_env_value VITE_VIDEOCHAT_ENABLE_WLVC true
set_env_value VITE_VIDEOCHAT_ENABLE_SFU false
...
set_env_value VIDEOCHAT_SPUTNIK_MAX_COUNT 25
set_env_value VIDEOCHAT_SPUTNIK_STALE_JOIN_MS 180000
