...
assert_domain_contract
log "BGF-07 proof commands: background contracts=(cd demo/video-chat/frontend-vue && npm run test:predeploy:background && node tests/contract/prod-debug-observability-contract.mjs); build=(cd demo/video-chat/frontend-vue && npm run build); deploy=demo/video-chat/scripts/deploy.sh (operator-run separately; deploy-smoke does not invoke it); routine post-deploy diagnostics=VIDEOCHAT_DEPLOY_DOMAIN=${DEPLOY_DOMAIN} VIDEOCHAT_PROD_DEBUG_SKIP_REMOTE=1 demo/video-chat/scripts/prod-debug.sh; optional domain/certificate smoke=VIDEOCHAT_DEPLOY_DOMAIN=${DEPLOY_DOMAIN} demo/video-chat/scripts/deploy-smoke.sh only when DNS/certbot validation is explicitly required; browser smoke=Chrome/Chromium and Firefox real call camera/audio/screenshare/reconnect/background-unavailable modal with prod-debug background fallback transition logs"
