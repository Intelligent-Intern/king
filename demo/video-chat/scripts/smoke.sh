...
if [[ "${VIDEOCHAT_SMOKE_SKIP_IAM_CI_GATE:-0}" != "1" ]]; then
  run_step "IAM call-access host-safe contract gate" bash -lc "'${ROOT_DIR}/scripts/iam-call-access-ci-gate.sh' --static"
else
  log "SKIP: IAM call-access host-safe contract gate disabled via VIDEOCHAT_SMOKE_SKIP_IAM_CI_GATE=1"
fi
