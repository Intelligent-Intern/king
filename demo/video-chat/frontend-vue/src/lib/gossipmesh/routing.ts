...
export function selectNeighbors(
  ...
  return shuffled.slice(0, Math.min(fanout, shuffled.length))
}
...
export function selectHealthyNeighbors(
  ...
  if (oneNew) {
    return [oneNew.id, ...withPressure.filter((p) => p.id !== oneNew.id).slice(0, 1)]
  }

  return withPressure.slice(0, 2).map((p) => p.id)
}
