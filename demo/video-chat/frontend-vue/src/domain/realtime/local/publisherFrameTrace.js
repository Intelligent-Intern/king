...
export function buildPublisherTransportStageMetrics({
  ...
  return {
    ...
    readback_frame_rate: Math.max(0, Number(videoProfile.readbackFrameRate || 0)),
    readback_interval_ms: Math.max(0, Number(videoProfile.readbackIntervalMs || videoProfile.encodeIntervalMs || 0)),
    keyframe_interval: Math.max(0, Number(videoProfile.keyFrameInterval || 0)),
    ...
    publisher_framing_mode: String(frameSize.framingMode || 'contain'),
    publisher_target_aspect_ratio: Math.max(0, Number(frameSize.targetAspectRatio || 0)),
