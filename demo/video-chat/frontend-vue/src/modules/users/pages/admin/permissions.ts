...
export function deriveAdminUserPermissions(user, currentUserId = 0) {
  ...
  const isSuperadmin = normalizeBoolean(user.is_superadmin, false);
  ...
  return {
    ...
    canChangeSuperadmin: normalizeBoolean(userPermissions.can_change_superadmin, false),
...
export function applyAdminUserPermissions(target, user, currentUserId = 0) {
  ...
  target.isSuperadmin = permissions.isSuperadmin;
  ...
  target.canChangeSuperadmin = permissions.canChangeSuperadmin;
...
export function resetAdminUserPermissions(target) {
  ...
  target.canChangeSuperadmin = false;
