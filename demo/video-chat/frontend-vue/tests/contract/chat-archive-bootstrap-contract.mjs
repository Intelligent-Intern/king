...
import { createCallWorkspaceChatArchiveBootstrap } from '../../src/domain/realtime/workspace/callWorkspace/chatArchiveBootstrap.js';
...
const bootstrap = createCallWorkspaceChatArchiveBootstrap({
  callbacks: {
  ...
  refs,
  options: { limit: 2, minIntervalMs: 0 },
});
...
assert.equal(diagnostics.at(-1)?.eventType, 'chat_archive_bootstrap_loaded');
...
assert.match(workspaceSource, /createCallWorkspaceChatArchiveBootstrap/, 'workspace must wire the chat archive bootstrap helper');
