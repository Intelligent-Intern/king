...
assert(
  /function pruneGossipNeighborForUserId\(userId,\s*reason = 'target_not_in_room'\)[\s\S]*assignedGossipNeighborIds\.delete\(peerId\);[\s\S]*gossipTopologyRepairRequestedAtByPeerId\.delete\(peerId\);[\s\S]*closeDedicatedGossipPeerConnection\(peerId,\s*reason\);/m.test(gossipDataLane),
  'gossip data lane must expose stale-neighbor pruning that removes assignment state and closes the dedicated gossip peer connection',
);
assert(
