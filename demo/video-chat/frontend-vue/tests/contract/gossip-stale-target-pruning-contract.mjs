...
assert(
  /else if \(expectedStaleTargetPublishFailure\)[\s\S]*level: 'warning'[\s\S]*eventType: 'realtime_signaling_stale_target_pruned'[\s\S]*code: 'target_not_in_room'[\s\S]*expected_stale_target_prune: true[\s\S]*signaling_reason: signalingError/m.test(socketLifecycle),
