...
try {
  ...
  requireContains(sfuTransport, "pressureReason = String(details?.reason || 'sfu_high_motion_payload_pressure')", 'transport preserves high-motion pressure reason');
  requireContains(sfuTransport, 'resetWlvcEncoderAfterDroppedEncodedFrame(pressureReason)', 'transport forces encoder recovery after payload drop');
  requireContains(sfuTransport, 'forced_next_keyframe: true', 'transport marks a forced recoverable keyframe plan');
