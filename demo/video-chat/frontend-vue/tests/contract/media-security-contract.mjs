...
try {
  ...
  assert.match(workspaceSource, /const marked = session\.markParticipantSet\(\[[\s\S]*normalizedSenderUserId,[\s\S]*\]\);[\s\S]*scheduleMediaSecurityParticipantSync\('hello_accepted', true\);/m, 'workspace must pin the hello sender into the participant set and force a follow-up rekey sync');
  assert.match(workspaceSource, /const accepted = await session\.handleSenderKeySignal\(normalizedSenderUserId, payloadBody \|\| \{\}\);[\s\S]*if \(!accepted\) \{[\s\S]*scheduleMediaSecurityParticipantSync\('sender_key_pending', true\);[\s\S]*\}/m, 'workspace must resync media security when a sender-key arrives before the local participant context is ready');
