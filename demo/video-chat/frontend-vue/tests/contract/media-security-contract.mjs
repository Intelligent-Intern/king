...
try {
  ...
  assert.match(mediaSecurityRuntimeSource, /if \(type === 'call\/media-security-sync-request'\) \{[\s\S]*session\.markParticipantSet\(mergeMediaSecurityParticipantIds\([\s\S]*normalizedSenderUserId,[\s\S]*\)\);[\s\S]*await sendMediaSecurityHello\(normalizedSenderUserId, true\);[\s\S]*scheduleMediaSecurityParticipantSync\('remote_sync_request', shouldForceRekey\);/m, 'media-security sync requests must refresh hello immediately, merge the requester into the participant set, and defer sender-key fanout until the authoritative room state settles');
  ...
  assert.match(mediaSecurityRuntimeSource, /requestRemoteMediaSecuritySync\(normalizedUserId, 'receiver_media_frame_error'\);[\s\S]*await sendMediaSecurityHello\(normalizedUserId, true\);[\s\S]*scheduleMediaSecurityParticipantSync\('publisher_recovery_request'\);/m, 'media-security recovery must ask the remote publisher for a fresh sender key and only retry local hello before settled participant sync');
