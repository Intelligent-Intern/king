...
try {
  ...
  assert.match(mediaSecurityRuntimeSource, /scheduleMediaSecurityParticipantSync\([\s\S]*'sender_key_participant_mismatch',[\s\S]*shouldForceRekeyAfterSenderKeyMiss\(normalizedTargetId\),[\s\S]*\);/m, 'sender-key participant mismatch must preserve already signaled receivers instead of forcing global video blanking');
  assert.match(mediaSecurityRuntimeSource, /requestRoomSnapshot\(\);[\s\S]*scheduleMediaSecurityParticipantSync\([\s\S]*'sender_key_participant_mismatch',[\s\S]*shouldForceRekeyAfterSenderKeyMiss\(normalizedTargetId\),[\s\S]*\);/m, 'sender-key participant mismatch must refresh the authoritative room snapshot before targeted rekeying');
  ...
  assert.match(mediaSecurityRuntimeSource, /const shouldForceRekeyAfterSignalFailure = errorCode === 'downgrade_attempt'[\s\S]*\|\| shouldForceRekeyAfterSenderKeyMiss\(normalizedSenderUserId\);/m, 'participant-set signal failures must force rekey only when no already signaled receiver can keep video alive');
