...
try {
  ...
  assert.match(mediaSecurityRuntimeSource, /scheduleMediaSecurityParticipantSync\('sender_key_participant_mismatch', true\);/, 'sender-key participant mismatch must force a fresh local sender-key epoch');
  assert.match(mediaSecurityRuntimeSource, /requestRoomSnapshot\(\);[\s\S]*scheduleMediaSecurityParticipantSync\('sender_key_participant_mismatch', true\);/, 'sender-key participant mismatch must refresh the authoritative room snapshot before rekeying');
  assert.match(mediaSecurityRuntimeSource, /scheduleMediaSecurityParticipantSync\('hello_participant_set_changed', true\);/, 'incoming hello participant-set changes must force a fresh local sender-key epoch');
  assert.match(mediaSecurityRuntimeSource, /const shouldForceRekeyAfterSignalFailure = true;/, 'participant-set and downgrade signal failures must both be treated as forced rekey recovery');
