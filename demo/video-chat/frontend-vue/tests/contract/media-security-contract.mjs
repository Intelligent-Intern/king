...
try {
  ...
  assert.match(mediaSecurityRuntimeSource, /function remoteMediaSecurityEligibleTargetIds\(\)[\s\S]*mediaSecurityEligibleTargetIds\(\)[\s\S]*normalizeRemoteMediaSecurityUserId\(userId\)/m, 'media-security runtime must normalize settled publisher-discovered targets through the remote-user guard');
  assert.match(mediaSecurityRuntimeSource, /const targetIds = remoteMediaSecurityEligibleTargetIds\(\);/, 'handshake timeout watchdog must only operate on settled remote publisher-discovered targets');
