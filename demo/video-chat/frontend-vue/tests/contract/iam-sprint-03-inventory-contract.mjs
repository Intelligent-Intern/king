...
const readiness = fs.readFileSync(
  path.join(repoRoot, 'documentation/archive/root-md-2026-05-10/READYNESS_TRACKER.md'),
  'utf8',
);
...
assert.match(
  ...
  'archived readiness tracker must record that Sprint 03 IAM intake is closed',
);
assert.match(
  backlog,
  /Sprint 03,[\s\S]*Sprint 04,[\s\S]*were completed before the 2026-05-10 root markdown cleanup/,
  'BACKLOG.md must retain Sprint 03 as historical IAM queue context after root cleanup',
);
assert.match(
  sprint,
  /## Sprint: IAM Call Access, Branch Hygiene, And Deploy Gate 11/,
  'SPRINT.md must contain only the active IAM sprint after root markdown cleanup',
);
assert.doesNotMatch(
  sprint,
