...
assert.match(relationStack, /\.crud-relation-search-field \.input\s*\{[\s\S]*?border-color:\s*var\(--color-border\);[\s\S]*?background-color:\s*var\(--color-border\);/, 'relation picker sidebar search input must use direct King styleguide token colors');
