...
try {
  ...
  requireContains(publisherPipeline, 'dispatchWlvcPublisherFrame({', 'publisher pipeline dispatches encoded frames through the live transport helper');
  requireContains(publisherFrameDispatch, 'const sent = await sendClient.sendEncodedFrame(frame);', 'publisher frame dispatch sends frames to live SFU client');
