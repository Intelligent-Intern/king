...
const auditEvents = read('backend-king-php/domain/audit/audit_events.php');
const iamAuditEvents = read('backend-king-php/domain/audit/iam_audit_events.php');

requireIncludes(auditEvents, "require_once __DIR__ . '/iam_audit_events.php';", 'core audit domain must load IAM audit helpers');
...
requireIncludes(ownerAbsence, 'const VIDEOCHAT_OWNER_ABSENCE_IMMUNE_OWNER_USER_ID = 1;', 'primary admin owner absence immunity');
requireIncludes(ownerAbsence, "'39c5b3ea-855b-40fd-b030-c8af1d512605'", 'permanent production call owner absence immunity');
...
requireIncludes(ownerAbsence, 'function videochat_realtime_owner_absence_owner_is_immune(int $ownerUserId): bool', 'owner absence immunity predicate');
requireIncludes(ownerAbsence, 'function videochat_realtime_owner_absence_call_is_immune(string $callId): bool', 'permanent call immunity predicate');
...
requireIncludes(ownerAbsence, 'videochat_realtime_owner_absence_disabled_payload($immuneStatus)', 'immune owner or permanent calls bypass owner absence timer');
requireIncludes(ownerAbsence, "'owner_absence_immune' => true", 'owner absence immune marker');
requireIncludes(ownerAbsence, "'owner_absence_immune_reason' => $immuneStatus", 'owner absence immune reason marker');
...
requireIncludes(ownerAbsence, 'videochat_audit_record_call_implicitly_ended($pdo, $snapshot', 'implicit owner-absence end audit write');
...
for (const helper of [
  'videochat_audit_record_owner_absence_timer_started',
  'videochat_audit_record_owner_absence_timer_cancelled',
  'videochat_audit_record_call_implicitly_ended',
]) {
  requireIncludes(iamAuditEvents, `function ${helper}(`, `${helper} helper`);
}
for (const eventType of [
  ...
  requireIncludes(iamAuditEvents, `'${eventType}'`, `${eventType} audit helper event`);
  requireIncludes(backendContract, `'${eventType}'`, `${eventType} backend proof assertion`);
}
...
for (const proofNeedle of [
  ...
  'owner return should audit-log timer cancellation',
  'owner timeout should audit-log implicit call end',
