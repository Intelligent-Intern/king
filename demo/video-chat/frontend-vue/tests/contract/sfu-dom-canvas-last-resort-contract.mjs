...
try {
  ...
  requireContains(policySource, 'DOM_CANVAS_COMPATIBILITY_MAX_FRAME_WIDTH = 320', 'DOM fallback max width cap');
  requireContains(policySource, 'DOM_CANVAS_COMPATIBILITY_MAX_FRAME_HEIGHT = 180', 'DOM fallback max height cap');
  requireContains(policySource, 'DOM_CANVAS_COMPATIBILITY_MAX_FPS = 6', 'DOM fallback FPS cap');
  ...
  requireContains(sourceReadback, 'resolveDomCanvasCompatibilityVideoFrameSize(source, activeProfile)', 'VideoFrame canvas fallback uses compatibility frame size');
  ...
  assert.equal(compatibilityProfile.frameWidth, 320);
  assert.equal(compatibilityProfile.frameHeight, 180);
  assert.equal(compatibilityProfile.captureFrameRate, 6);
  assert.ok(compatibilityProfile.encodeIntervalMs >= 167);
  const portrait = policy.resolveDomCanvasCompatibilityVideoFrameSize({
    displayWidth: 720,
    displayHeight: 1280,
  }, profile);
  assert.equal(portrait.frameHeight, 180);
  assert.ok(portrait.frameWidth < 180, 'portrait DOM fallback must preserve aspect ratio inside capped frame');
