...
assert.match(
  accountConfirmation,
  /function videochat_build_call_access_account_confirmation_url\(string \$token, array \$options = \[\]\)[\s\S]*account-update-confirmation\?[\s\S]*call_access_account_update_confirmation_token/s,
  'confirmation URLs must carry only the account-update confirmation token on the safe origin',
);
assert.match(
