...
requireContains(controller, "rolloutStrategy || options.mediaCarrierMode || 'sfu_mirror'", 'telemetry snapshots must default to conservative SFU mirror carrier rollout')
...
requireContains(workspaceGossip, 'mediaCarrierMode: MEDIA_CARRIER_CONFIG.mode', 'workspace telemetry snapshots must send the explicit media carrier mode')
requireContains(workspaceGossip, 'rolloutStrategy: MEDIA_CARRIER_CONFIG.mode', 'workspace telemetry snapshots must bind rollout strategy to the active media carrier')
