...
const workspaceConfig = fs.readFileSync(path.join(root, 'src/domain/realtime/workspace/config.ts'), 'utf8');
const lifecycle = fs.readFileSync(path.join(root, 'src/domain/realtime/workspace/callWorkspace/lifecycle.ts'), 'utf8');
const mediaOrchestration = fs.readFileSync(path.join(root, 'src/domain/realtime/local/mediaOrchestration.ts'), 'utf8');
const publisherPipeline = fs.readFileSync(path.join(root, 'src/domain/realtime/local/publisherPipeline.ts'), 'utf8');
const runtimeSwitching = fs.readFileSync(path.join(root, 'src/domain/realtime/workspace/callWorkspace/runtimeSwitching.ts'), 'utf8');
...
try {
  ...
  assert.match(callSettingsMarkup, /<label for="call-left-layout-mode">\{\{ t\('calls\.workspace\.video_layout'\) \}\}<\/label>[\s\S]*<AppSelect[\s\S]*id="call-left-layout-mode"/, 'layout mode must use existing AppSelect styling');
  assert.match(callSettingsMarkup, /<label for="call-left-layout-strategy">\{\{ t\('calls\.workspace\.activity_strategy'\) \}\}<\/label>[\s\S]*<AppSelect[\s\S]*id="call-left-layout-strategy"/, 'activity strategy must use existing AppSelect styling');
