...
try {
  ...
  assert.equal(width, 180, 'mini story width must preserve enough horizontal video context');
  assert.equal(height, 112, 'mini story height must keep the strip compact while avoiding square video jumps');
  assert.ok(width > height, 'mini story dimensions must be rectangular instead of square');
  assert.equal(stripHeight, 132, 'mini strip height must fit the rectangular tile plus vertical padding');
  assert.match(miniTileBlock, /border-radius:\s*0;/, 'mini tiles must have square corners');
  assert.match(stageCss, /\.workspace-mini-tile\s*\{[\s\S]*aspect-ratio:\s*16\s*\/\s*10;/, 'mini tiles must declare a stable rectangular aspect ratio');
  ...
  assert.match(panelsCss, /@media \(max-width: 760px\) and \(orientation: landscape\) \{[\s\S]*--mini-strip-height:\s*100px;[\s\S]*--mini-story-width:\s*142px;[\s\S]*--mini-story-height:\s*80px;/, 'mobile landscape mini strip must use compact rectangular tiles');
