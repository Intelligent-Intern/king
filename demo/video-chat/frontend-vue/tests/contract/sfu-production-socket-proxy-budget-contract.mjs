...
try {
  ...
  requireContains(probe, 'QUALITY_MAX_PAYLOAD_BYTES = 1280 * 1024', 'probe exercises full quality profile payload budget');
  requireContains(probe, 'QUALITY_MAX_BUFFERED_BYTES = 1536 * 1024', 'probe enforces quality bufferedAmount budget');
  requireContains(probe, 'CRITICAL_BUFFERED_BYTES = 5 * 1024 * 1024', 'probe fails before critical backpressure');
