...
const routeSourcePaths = workspaceModuleRouteRecords
  ...
  .filter(Boolean);
assert.ok(routeSourcePaths.length >= expectedModuleRoutes.size, 'module route records must expose descriptor source paths');
...
const excludedRouteSources = routeSourcePaths.filter((sourcePath) => excludedPatterns.some((pattern) => pattern.test(`demo/video-chat/frontend-vue/src/${sourcePath}`)));
assert.deepEqual(excludedRouteSources, [], 'module descriptors must not route governance/administration pages through call/realtime internals');

if (process.env.MODULE_REFACTOR_STRICT_DIFF === '1') {
