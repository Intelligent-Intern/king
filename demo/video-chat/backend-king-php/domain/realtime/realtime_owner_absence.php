...
function videochat_realtime_owner_absence_snapshot(PDO $pdo, string $callId, string $roomId, ?int $nowMs = null): array
{
    ...
    $presenceRows = videochat_realtime_owner_absence_active_presence(
        $pdo,
        (string) $call['id'],
        (string) $call['room_id'],
        $effectiveNowMs
...
function videochat_realtime_owner_absence_revoke_call_access_sessions(PDO $pdo, string $callId, string $revokedAt): int
{
    if (
        $callId === ''
        || !function_exists('videochat_tenant_table_has_column')
        || !videochat_tenant_table_has_column($pdo, 'call_access_sessions', 'call_id')
    ) {
        return 0;
    }
...
function videochat_realtime_owner_absence_disable_call_access_links(PDO $pdo, string $callId, string $disabledAt): int
{
    if (
        $callId === ''
        || !function_exists('videochat_tenant_table_has_column')
...
function videochat_realtime_owner_absence_clear_presence(PDO $pdo, string $callId): int
{
    if ($callId === '') {
        return 0;
    }
    ...
    videochat_realtime_presence_db_bootstrap($pdo);
    $statement = $pdo->prepare('DELETE FROM realtime_presence_connections WHERE call_id = :call_id');
...
function videochat_realtime_apply_owner_absence_timeout(PDO $pdo, string $callId, string $roomId, ?int $nowMs = null): array
{
    ...
    $snapshotCallId = (string) ($snapshot['call_id'] ?? $callId);
    $snapshotRoomId = (string) ($snapshot['room_id'] ?? $roomId);
...
UPDATE call_participants
SET left_at = CASE
        WHEN joined_at IS NOT NULL AND (left_at IS NULL OR left_at = '') THEN :left_at
        ELSE left_at
    END
WHERE call_id = :call_id
SQL
    ...
    $lifecycle = [
        'ok' => true,
        'reason' => $transitioned ? 'owner_absent_timeout' : 'already_terminal',
        'transition' => 'ended',
...
function videochat_realtime_owner_absence_downgrade_absent_owner_connection(
    PDO $pdo,
    array $connection,
    ?int $nowMs = null
): array {
    ...
    $ownerAbsence = videochat_realtime_owner_absence_snapshot($pdo, $callId, $roomId, $nowMs);
    if (
        !(bool) ($ownerAbsence['enabled'] ?? false)
        || (int) ($ownerAbsence['owner_user_id'] ?? 0) !== $userId
        || (bool) ($ownerAbsence['owner_present'] ?? false)
    ) {
        return $connection;
    }

    $connection['call_role'] = 'participant';
    $connection['effective_call_role'] = 'participant';
    $connection['can_moderate_call'] = false;
    $connection['can_manage_call_owner'] = false;
    if ((string) ($ownerAbsence['call_status'] ?? '') === 'ended') {
        $connection['active_call_id'] = '';
    }
