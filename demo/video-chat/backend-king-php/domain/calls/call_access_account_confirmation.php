...
function videochat_call_access_record_account_confirmation_failure(PDO $pdo, array $row, string $reason): void
{
    videochat_audit_record_event($pdo, [
        'tenant_id' => is_numeric($row['tenant_id'] ?? null) ? (int) $row['tenant_id'] : null,
        'event_type' => 'call_access_account_update_confirmation_failed',
        'actor_user_id' => is_numeric($row['user_id'] ?? null) ? (int) $row['user_id'] : null,
        'target_user_id' => is_numeric($row['user_id'] ?? null) ? (int) $row['user_id'] : null,
...
function videochat_call_access_confirm_account_update(PDO $pdo, string $token, int $authenticatedUserId = 0): array
{
    ...
    $query = $pdo->prepare(
        <<<'SQL'
SELECT id, token_fingerprint, tenant_id, call_id, access_fingerprint, user_id, pending_payload_json, expires_at, consumed_at
FROM call_access_account_update_confirmations
...
SQL
    ...
    if ($authenticatedUserId > 0 && $authenticatedUserId !== $userId) {
        videochat_call_access_record_account_confirmation_failure($pdo, $row, 'account_bound');
        return [
            'ok' => false,
            'reason' => 'forbidden',
    ...
    if ($existingConsumedAt !== '') {
        videochat_call_access_record_account_confirmation_failure($pdo, $row, 'already_consumed');
        return [
            'ok' => false,
            'reason' => 'conflict',
    ...
    if (!is_int($expiresAtUnix) || $expiresAtUnix <= time()) {
        videochat_call_access_record_account_confirmation_failure($pdo, $row, 'expired');
        return [
            'ok' => false,
            'reason' => 'expired',
    ...
    try {
        $consumedAt = gmdate('c');
        $consume = $pdo->prepare(
            'UPDATE call_access_account_update_confirmations SET consumed_at = :consumed_at WHERE token_fingerprint = :token_fingerprint AND (consumed_at IS NULL OR trim(consumed_at) = \'\')'
        );
        $consume->execute([
