...
function videochat_issue_session_for_call_access(
    ...
    $requiresGuestName = $linkKind !== 'open' && videochat_call_access_requires_guest_name($accessLink, $targetUser);
    ...
    if ($linkKind === 'open') {
        ...
        if (!$openLinkUsesAuthenticatedUser) {
    ...
    if ($linkKind === 'personal' && !$createdPersonalGuest && $verifiedUserId > 0 && $verifiedUserId !== $userId) {
        ...
        if (function_exists('videochat_call_access_record_duplicate_personalized_link_review')) {
    ...
    if ($linkKind === 'personal' && !$createdPersonalGuest && $authenticatedUserId > 0 && $authenticatedUserId !== $userId) {
        ...
        if ($hostName !== '' && function_exists('videochat_call_access_host_verification_rate_limit')) {
    ...
    if ($linkKind === 'open') {
        ...
        if ($openLinkUsesAuthenticatedUser && !$openLinkUsesOwnDirectRights) {
            videochat_ensure_internal_call_participant(
                $pdo,
                (string) ($call['id'] ?? ''),
                $userId,
        ...
        } elseif (!$openLinkUsesOwnDirectRights && $openInviteOnlyLink) {
            $callDecision = videochat_call_access_decision_result(
                true,
                'allowed',
                'anonymous_link_lobby',
