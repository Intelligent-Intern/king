...
function videochat_tenancy_governance_user_payload_has_roles(array $payload): bool
{
    $relationships = is_array($payload['relationships'] ?? null) ? $payload['relationships'] : [];
    return array_key_exists('roles', $relationships) || array_key_exists('roles', $payload);
}

function videochat_tenancy_governance_user_role_values(array $payload): array
{
    $relationships = is_array($payload['relationships'] ?? null) ? $payload['relationships'] : [];
    $roles = array_key_exists('roles', $relationships) ? $relationships['roles'] : ($payload['roles'] ?? []);
    return is_array($roles) ? $roles : [];
...
function videochat_tenancy_governance_validate_user_roles(PDO $pdo, int $tenantId, array $payload): array
{
    if (!videochat_tenancy_governance_user_payload_has_roles($payload)) {
        return ['ok' => true, 'role_ids' => []];
    }
    $roleIds = [];
    foreach (videochat_tenancy_governance_user_role_values($payload) as $value) {
        $identifier = videochat_tenancy_governance_group_role_identifier($value);
        $role = videochat_tenancy_fetch_governance_role($pdo, $tenantId, $identifier);
        if (!is_array($role)) {
            return ['ok' => false, 'errors' => ['roles' => 'not_found']];
...
function videochat_tenancy_governance_user_ids_for_role(PDO $pdo, int $tenantId, int $roleId): array
{
    if ($tenantId <= 0 || $roleId <= 0) {
        return [];
    }
    $query = $pdo->prepare('SELECT user_id FROM governance_user_roles WHERE tenant_id = :tenant_id AND role_id = :role_id');
    $query->execute([':tenant_id' => $tenantId, ':role_id' => $roleId]);
    return array_values(array_unique(array_filter(array_map('intval', $query->fetchAll(PDO::FETCH_COLUMN) ?: []), static fn (int $id): bool => $id > 0)));
}

function videochat_tenancy_governance_replace_user_roles(PDO $pdo, int $tenantId, int $userId, array $roleIds): void
{
    $delete = $pdo->prepare('DELETE FROM governance_user_roles WHERE tenant_id = :tenant_id AND user_id = :user_id');
    $delete->execute([':tenant_id' => $tenantId, ':user_id' => $userId]);
    $insert = $pdo->prepare('INSERT OR IGNORE INTO governance_user_roles(tenant_id, user_id, role_id) VALUES(:tenant_id, :user_id, :role_id)');
    foreach ($roleIds as $roleId) {
        $insert->execute([':tenant_id' => $tenantId, ':user_id' => $userId, ':role_id' => (int) $roleId]);
    }
}

function videochat_tenancy_governance_user_role_rows(PDO $pdo, int $tenantId, array $userIds): array
{
    ...
    $query = $pdo->prepare(sprintf(
        'SELECT user_id, governance_roles.public_id, governance_roles.key, governance_roles.name, governance_roles.status FROM governance_user_roles INNER JOIN governance_roles ON governance_roles.id = governance_user_roles.role_id WHERE governance_user_roles.tenant_id = :tenant_id AND user_id IN (%s) ORDER BY lower(governance_roles.name) ASC',
        implode(', ', $placeholders)
    ));
    $query->execute($params);
    $rows = array_fill_keys($ids, []);
    foreach ($query->fetchAll(PDO::FETCH_ASSOC) ?: [] as $row) {
        $rows[(int) ($row['user_id'] ?? 0)][] = [
            'entity_key' => 'roles',
            'id' => (string) ($row['public_id'] ?? ''),
            'key' => trim((string) ($row['key'] ?? '')) !== '' ? (string) $row['key'] : (string) ($row['public_id'] ?? ''),
            'name' => (string) ($row['name'] ?? ''),
...
function videochat_tenancy_governance_enrich_user_role_relationships(PDO $pdo, int $tenantId, array $row): array
{
    $userId = (int) ($row['id'] ?? 0);
    $roles = videochat_tenancy_governance_user_role_rows($pdo, $tenantId, [$userId]);
    $row['relationships'] = [
        ...(is_array($row['relationships'] ?? null) ? $row['relationships'] : []),
        'roles' => $roles[$userId] ?? [],
    ];
...
function videochat_tenancy_governance_enrich_user_role_rows(PDO $pdo, int $tenantId, array $rows): array
{
    $userIds = array_map(static fn (array $row): int => (int) ($row['id'] ?? 0), $rows);
    $roles = videochat_tenancy_governance_user_role_rows($pdo, $tenantId, $userIds);
    return array_map(static function (array $row) use ($roles): array {
        $userId = (int) ($row['id'] ?? 0);
        $row['relationships'] = [
            ...(is_array($row['relationships'] ?? null) ? $row['relationships'] : []),
...
function videochat_tenancy_governance_user_role_permission_rows(PDO $pdo, int $tenantId, int $userId): array
{
    $query = $pdo->prepare(
        <<<'SQL'
SELECT governance_role_permissions.permission_key, governance_role_permissions.resource_type, '*' AS resource_id, governance_role_permissions.action
FROM governance_user_roles
INNER JOIN governance_roles ON governance_roles.id = governance_user_roles.role_id
INNER JOIN governance_role_permissions ON governance_role_permissions.role_id = governance_roles.id
WHERE governance_user_roles.tenant_id = :tenant_id
  AND governance_user_roles.user_id = :user_id
  AND governance_roles.status = 'active'
  AND governance_role_permissions.tenant_id = :tenant_id
UNION
SELECT 'module.' || governance_role_modules.module_key || '.read' AS permission_key, 'module' AS resource_type, governance_role_modules.module_key AS resource_id, 'read' AS action
...
function videochat_tenancy_governance_sync_user_role_grants(PDO $pdo, int $tenantId, int $userId, int $actorUserId): array
{
    if ($tenantId <= 0 || $userId <= 0) {
        return ['ok' => false, 'errors' => ['user' => 'not_found']];
    }
    $delete = $pdo->prepare("DELETE FROM permission_grants WHERE tenant_id = :tenant_id AND subject_type = 'user' AND user_id = :user_id AND source = 'user_roles'");
    $delete->execute([':tenant_id' => $tenantId, ':user_id' => $userId]);
    $exists = $pdo->prepare(
        <<<'SQL'
SELECT id
...
SQL
    ...
    $insert = $pdo->prepare(
        <<<'SQL'
INSERT INTO permission_grants(
    tenant_id, public_id, resource_type, resource_id, action, subject_type, user_id,
    created_by_user_id, label, description, permission_key, source, created_at, updated_at
) VALUES(
    :tenant_id, :public_id, :resource_type, :resource_id, :action, 'user', :user_id,
    :created_by_user_id, :label, '', :permission_key, 'user_roles', :created_at, :updated_at
)
SQL
    );
    $now = gmdate('c');
    foreach (videochat_tenancy_governance_user_role_permission_rows($pdo, $tenantId, $userId) as $permission) {
        $exists->execute([
            ':tenant_id' => $tenantId,
            ':user_id' => $userId,
            ':resource_type' => (string) $permission['resource_type'],
...
function videochat_tenancy_governance_clear_user_roles(PDO $pdo, int $tenantId, int $userId): array
{
    if ($tenantId <= 0 || $userId <= 0) {
        return ['ok' => false, 'errors' => ['user' => 'not_found']];
    }
    $deleteAssignments = $pdo->prepare('DELETE FROM governance_user_roles WHERE tenant_id = :tenant_id AND user_id = :user_id');
    $deleteAssignments->execute([':tenant_id' => $tenantId, ':user_id' => $userId]);
    $deleteGrants = $pdo->prepare("DELETE FROM permission_grants WHERE tenant_id = :tenant_id AND subject_type = 'user' AND user_id = :user_id AND source = 'user_roles'");
    $deleteGrants->execute([':tenant_id' => $tenantId, ':user_id' => $userId]);
    return ['ok' => true];
}

function videochat_tenancy_governance_sync_users_for_role(PDO $pdo, int $tenantId, int $roleId, int $actorUserId): array
{
    foreach (videochat_tenancy_governance_user_ids_for_role($pdo, $tenantId, $roleId) as $userId) {
        $sync = videochat_tenancy_governance_sync_user_role_grants($pdo, $tenantId, $userId, $actorUserId);
        if (!(bool) ($sync['ok'] ?? false)) {
            return $sync;
        }
    }
...
function videochat_tenancy_governance_sync_user_roles(PDO $pdo, int $tenantId, int $userId, int $actorUserId, array $payload): array
{
    if (!videochat_tenancy_governance_user_payload_has_roles($payload)) {
        return ['ok' => true];
    }
    $validation = videochat_tenancy_governance_validate_user_roles($pdo, $tenantId, $payload);
    if (!(bool) ($validation['ok'] ?? false)) {
        return $validation;
    }
    videochat_tenancy_governance_replace_user_roles($pdo, $tenantId, $userId, (array) $validation['role_ids']);
    return videochat_tenancy_governance_sync_user_role_grants($pdo, $tenantId, $userId, $actorUserId);
}
