#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BACKEND_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
REPO_ROOT="$(cd "${BACKEND_ROOT}/../../.." && pwd)"
PHP_BIN="${PHP_BIN:-php}"
DOCKER_BIN="${DOCKER_BIN:-docker}"
DOCKER_IMAGE="${IAM_GUEST_LIST_MEMBERSHIP_PHP_IMAGE:-php:8.4-cli-trixie}"
LOG_PREFIX="[call-access-guest-list-membership-docker-proof]"

CONTRACTS=(
  "call-guest-list-direct-join-contract.sh"
  "call-access-membership-removal-contract.sh"
)

php_has_pdo_sqlite() {
  "${PHP_BIN}" -m 2>/dev/null | grep -qi '^pdo_sqlite$'
}

run_contracts_with_php_bin() {
  echo "${LOG_PREFIX} PHP runtime: $("${PHP_BIN}" -r 'echo PHP_VERSION;' 2>/dev/null)"
  "${PHP_BIN}" -m | grep -i '^pdo_sqlite$'
  for contract in "${CONTRACTS[@]}"; do
    echo "${LOG_PREFIX} ${contract}"
    PHP_BIN="${PHP_BIN}" "${SCRIPT_DIR}/${contract}"
  done
}

run_contracts_with_docker() {
  if ! command -v "${DOCKER_BIN}" >/dev/null 2>&1; then
    echo "${LOG_PREFIX} FAIL: ${PHP_BIN} lacks pdo_sqlite and ${DOCKER_BIN} is unavailable" >&2
    exit 1
  fi

  echo "${LOG_PREFIX} Host PHP lacks pdo_sqlite; using container fallback"
  echo "${LOG_PREFIX} Container runtime: ${DOCKER_IMAGE}"
  "${DOCKER_BIN}" run --rm \
    -v "${REPO_ROOT}:/workspace" \
    -w /workspace/demo/video-chat/backend-king-php \
    -e PHP_BIN=php \
    "${DOCKER_IMAGE}" \
    bash -lc '
      set -euo pipefail
      if ! php -m | grep -qi "^pdo_sqlite$"; then
        if command -v docker-php-ext-install >/dev/null 2>&1; then
          export DEBIAN_FRONTEND=noninteractive
          apt-get update >/tmp/iam-guest-list-membership-apt-update.log
          apt-get install -y --no-install-recommends libsqlite3-dev >/tmp/iam-guest-list-membership-apt-install.log
          docker-php-ext-install pdo_sqlite >/tmp/iam-guest-list-membership-pdo-build.log
        fi
      fi
      php -m | grep -i "^pdo_sqlite$"
      tests/call-guest-list-direct-join-contract.sh
      tests/call-access-membership-removal-contract.sh
    '
}

if php_has_pdo_sqlite; then
  run_contracts_with_php_bin
else
  run_contracts_with_docker
fi

echo "${LOG_PREFIX} PASS"
