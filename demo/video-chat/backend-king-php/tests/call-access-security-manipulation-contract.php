...
try {
    ...
    videochat_call_access_security_manipulation_assert(videochat_call_access_account_confirmation_bootstrap($pdo), 'confirmation table should bootstrap for post-reject inspection');
    ...
    videochat_call_access_security_manipulation_assert(
        (int) ($allowedRequest['status'] ?? 0) === 200,
        'allowed frontend origin should create account confirmation: status=' . (string) ($allowedRequest['status'] ?? '') . ' body=' . (string) ($allowedRequest['body'] ?? '')
    );
    ...
    videochat_call_access_security_manipulation_assert(
        in_array((string) ($deletedAuth['reason'] ?? ''), ['revoked_session', 'call_access_binding_mismatch', 'call_access_link_invalidated', 'call_access_call_not_joinable'], true),
        'deleted call-access auth reason mismatch: ' . (string) ($deletedAuth['reason'] ?? '')
    );
