...
try {
    ...
    $orgAMemberTenantBAdminId = videochat_call_access_cross_org_create_user($pdo, 'cross-org-a-member-beta-admin@example.test', 'Org A Member Beta Admin');
    ...
    videochat_call_access_cross_org_attach_user($pdo, $tenantAId, $orgAMemberTenantBAdminId, 'member', false);
    videochat_call_access_cross_org_attach_user($pdo, $tenantBId, $orgAMemberTenantBAdminId, 'admin', true);
    ...
    videochat_call_access_cross_org_attach_organization($pdo, $tenantAId, $organizationAId, $orgAMemberTenantBAdminId, 'member');
    videochat_call_access_cross_org_attach_organization($pdo, $tenantBId, $organizationBId, $orgAMemberTenantBAdminId, 'admin');
    ...
    videochat_ensure_internal_call_participant(
        $pdo,
        $orgACallId,
        $orgAMemberTenantBAdminId,
        'cross-org-a-member-beta-admin@example.test',
        'Org A Member Beta Admin',
        'invited'
    );

    $orgAOwnPersonalLink = videochat_create_call_access_link_for_user($pdo, $orgACallId, $orgAAdminId, 'user', [
        'link_kind' => 'personal',
    ...
    $multiOrgPersonalAccessId = videochat_call_access_cross_org_insert_link($pdo, $tenantAId, $orgACallId, $orgAMemberTenantBAdminId);
    $multiOrgPersonalSession = videochat_issue_session_for_call_access(
        ...
        ['client_ip' => '127.0.0.1', 'user_agent' => 'call-access-cross-org-contract'],
        [
            'authenticated_user_id' => $orgAMemberTenantBAdminId,
            'authenticated_session_id' => 'sess_cross_org_multi_active_b',
            'verified_user_id' => $orgAMemberTenantBAdminId,
    ...
    $multiOrgCallDecision = videochat_decide_call_access_for_user($pdo, $orgACallId, $orgAMemberTenantBAdminId, 'user', $tenantAId);
    ...
    videochat_call_access_cross_org_assert((string) ($multiOrgCallDecision['source'] ?? '') === 'internal_participant', 'multi-organization account should keep organization A participant source only for organization A call');
    videochat_call_access_cross_org_assert((bool) ($multiOrgCallDecision['can_moderate'] ?? true) === false, 'multi-organization account must not carry organization B moderation rights into organization A call');
...
SQL
    ...
    videochat_call_access_cross_org_assert((string) (($foreignTargetSession['errors'] ?? [])['auth'] ?? '') === 'not_bound_to_current_user', 'foreign personalized-link mismatch must not bind the authenticated user');
    videochat_call_access_cross_org_assert((string) (($foreignTargetSession['errors'] ?? [])['host_name'] ?? '') === 'wrong_host_name', 'foreign personalized-link mismatch must fail closed on wrong host verification');
...
SQL
    ...
    $temporaryCreate = videochat_create_guest_user_for_call_access($pdo, 'Org A Temporary Invitee', $tenantAId);
    ...
    videochat_call_access_cross_org_assert(videochat_tenant_user_is_member($pdo, $temporaryUserId, $tenantAId), 'organization A temporary invitation account should be scoped to organization A tenant');
