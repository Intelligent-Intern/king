...
SQL
    ...
    $lateEndedUnrelatedAccessId = videochat_call_lifecycle_contract_insert_late_retained_link($pdo, $endCallId, $adminUserId, $unrelatedGuestId, $tenantId);
    $lateEndedUnrelatedResolve = videochat_resolve_call_access_public($pdo, $lateEndedUnrelatedAccessId);
    videochat_call_lifecycle_contract_assert(!(bool) ($lateEndedUnrelatedResolve['ok'] ?? true), 'late retained ended unrelated-user link must not resolve');
    videochat_call_lifecycle_contract_assert((string) ($lateEndedUnrelatedResolve['reason'] ?? '') === 'conflict', 'late retained ended unrelated-user link should expose safe ended-call conflict');
    videochat_call_lifecycle_contract_assert((string) (videochat_call_lifecycle_contract_user($pdo, $unrelatedGuestId)['status'] ?? '') === 'active', 'end must not disable unrelated temp guest through late retained link');
