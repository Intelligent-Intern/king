...
try {
    ...
    $organizationId = isset($organizationRow['id']) && is_numeric($organizationRow['id']) ? (int) $organizationRow['id'] : 0;
    if ($organizationId <= 0 && $organizationPublicId !== '') {
    ...
    if ($organizationPublicId === '' && $organizationId > 0) {
        $organizationPublicIdQuery = $pdo->prepare('SELECT public_id FROM organizations WHERE tenant_id = :tenant_id AND id = :id LIMIT 1');
        $organizationPublicIdQuery->execute([':tenant_id' => $tenantId, ':id' => $organizationId]);
        $organizationPublicId = (string) $organizationPublicIdQuery->fetchColumn();
    }
    ...
    videochat_iam_core_org_session_assert(
        $organizationId > 0,
        'created organization should be persisted for public id ' . ($organizationPublicId !== '' ? $organizationPublicId : '[missing]')
    );
...
SQL
    ...
    unset($roleUpdate, $membershipRole, $tenantOnlyOrganizationCount);
    ...
    videochat_iam_core_org_session_assert(
        (bool) ($ownedCall['ok'] ?? false),
        'organization member should create an invite-only call: ' . json_encode([
            'reason' => (string) ($ownedCall['reason'] ?? 'unknown'),
            'errors' => $ownedCall['errors'] ?? [],
