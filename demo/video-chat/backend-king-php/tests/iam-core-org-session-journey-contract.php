...
function videochat_iam_core_org_session_seed_call(
    PDO $pdo,
    int $tenantId,
    int $ownerUserId,
    string $title,
    ...
    $ownerQuery = $pdo->prepare('SELECT id, email, display_name FROM users WHERE id = :id AND status = :status LIMIT 1');
    $ownerQuery->execute([':id' => $ownerUserId, ':status' => 'active']);
    $owner = $ownerQuery->fetch(PDO::FETCH_ASSOC);
    videochat_iam_core_org_session_assert(is_array($owner), 'call fixture owner should be an active user');

    $callId = 'iam-core-' . bin2hex(random_bytes(8));
    $createdAt = gmdate('c');
    $startsAtUnix = strtotime($startsAt);
    $endsAtUnix = strtotime($endsAt);
    $initialStatus = is_int($startsAtUnix) && is_int($endsAtUnix) && $startsAtUnix <= time() && time() < $endsAtUnix
        ? 'active'
        : 'scheduled';
    $durationMinutes = is_int($startsAtUnix) && is_int($endsAtUnix)
        ? max(0, (int) (($endsAtUnix - $startsAtUnix) / 60))
        : 0;
    ...
    try {
        $insertRoom = $pdo->prepare(
            <<<'SQL'
INSERT INTO rooms(id, name, visibility, status, created_by_user_id, created_at, updated_at, tenant_id)
VALUES(:id, :name, 'private', 'active', :created_by_user_id, :created_at, :updated_at, :tenant_id)
SQL
        );
        $insertRoom->execute([
            ':id' => $callId,
...
INSERT INTO calls(
    ...
    cancelled_at, cancel_reason, cancel_message, created_at, updated_at, tenant_id
) VALUES(
    :id, :room_id, :title, :access_mode, :owner_user_id, :status, :starts_at, :ends_at,
    'UTC', :schedule_date, :schedule_duration_minutes, 0,
    NULL, NULL, NULL, :created_at, :updated_at, :tenant_id
)
SQL
        );
        $insertCall->execute([
            ...
            ':starts_at' => $startsAt,
            ':ends_at' => $endsAt,
            ':schedule_date' => gmdate('Y-m-d', is_int($startsAtUnix) ? $startsAtUnix : time()),
            ':schedule_duration_minutes' => $durationMinutes,
            ':created_at' => $createdAt,
            ':updated_at' => $createdAt,
            ':tenant_id' => $tenantId,
        ]);
...
SQL
        ...
        videochat_iam_core_org_session_assert(false, 'call fixture should persist: ' . $error->getMessage());
    }
...
try {
    ...
    $organizationPublicId = (string) (($organizationRow['public_id'] ?? '') !== '' ? $organizationRow['public_id'] : ($organizationRow['id'] ?? ''));
    $organizationDatabaseId = (int) ($organizationRow['database_id'] ?? (ctype_digit((string) ($organizationRow['id'] ?? '')) ? (int) $organizationRow['id'] : 0));
    videochat_iam_core_org_session_assert($organizationPublicId !== '', 'organization public id should be present');

    $organizationIdQuery = $pdo->prepare(
        'SELECT id FROM organizations WHERE tenant_id = :tenant_id AND (public_id = :public_id OR id = :database_id) LIMIT 1'
    );
    $organizationIdQuery->execute([
        ':tenant_id' => $tenantId,
    ...
    if ($organizationId <= 0) {
        $organizationNameQuery = $pdo->prepare(
            'SELECT id FROM organizations WHERE tenant_id = :tenant_id AND name = :name AND status = :status ORDER BY id DESC LIMIT 1'
        );
        $organizationNameQuery->execute([
            ':tenant_id' => $tenantId,
            ':name' => 'IAM Core Contract Organization',
            ':status' => 'active',
        ]);
        $organizationId = (int) $organizationNameQuery->fetchColumn();
    }
...
SQL
    ...
    $ownedCallId = videochat_iam_core_org_session_seed_call(
        $pdo,
        $tenantId,
        $normalUser['id'],
        'IAM Core Organization Rights Call',
        'invite_only',
        $startsAt,
        $endsAt
    );
    ...
    $openCallId = videochat_iam_core_org_session_seed_call(
        $pdo,
        $tenantId,
        $normalUser['id'],
        'IAM Core Logged In Open Link Call',
        'free_for_all',
        $startsAt,
        $endsAt
    );
