...
try {
    ...
    videochat_iam1117_assert((string) ($initialTempResolution['initial_room_id'] ?? '') === $tempRoomId, 'free-for-all temp guest should enter before kick');
    videochat_iam1117_assert((string) ($initialTempResolution['pending_room_id'] ?? '') === '', 'free-for-all temp guest should not wait before kick');
    videochat_iam1117_assert(videochat_iam1117_invite_state($pdo, $tempCallId, $tempUserId) === 'allowed', 'free-for-all temp guest should persist initial allowed invite state');
    ...
    videochat_iam1117_connection($pdo, $tempPresenceState, $tempRoomId, $tempCallId, $tempUserId, 'IAM11-17 Temp Guest', 'user', 'iam1117-temp-admitted', $tenantId, $tempSessionId, false);
    ...
    $tempWaitingConnection = videochat_iam1117_connection($pdo, $tempPresenceState, $tempRoomId, $tempCallId, $tempUserId, 'IAM11-17 Temp Guest', 'user', 'iam1117-temp-waiting', $tenantId, $tempSessionId, true);
