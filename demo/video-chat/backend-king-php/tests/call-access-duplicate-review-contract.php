...
function videochat_call_access_duplicate_review_spawn_process_worker(
    string $databasePath,
    string $accessId,
    string $startPath,
    string $resultPath,
    ...
    videochat_call_access_duplicate_review_assert(function_exists('proc_open'), 'proc_open should be available for process race fallback');

    $env = [
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_WORKER' => '1',
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_DATABASE' => $databasePath,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_ACCESS_ID' => $accessId,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_START' => $startPath,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_RESULT' => $resultPath,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_SESSION_ID' => $issuedSessionId,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_USER_ID' => (string) $userId,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_AUTH_SESSION_ID' => $authSessionId,
        'VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_LABEL' => $label,
    ];
    foreach (['PATH', 'PHPRC', 'PHP_INI_SCAN_DIR'] as $key) {
        $value = getenv($key);
    ...
    $process = proc_open([PHP_BINARY, __FILE__], $descriptors, $pipes, __DIR__, $env);
    videochat_call_access_duplicate_review_assert(is_resource($process), "{$label} process worker should start");
    fclose($pipes[0]);

    return ['process' => $process, 'pipes' => $pipes];
}

function videochat_call_access_duplicate_review_wait_process_worker(array $worker, string $label): void
{
    $pipes = is_array($worker['pipes'] ?? null) ? $worker['pipes'] : [];
    ...
    videochat_call_access_duplicate_review_assert(is_resource($process), "{$label} process worker should be valid");
    $exitCode = proc_close($process);
    videochat_call_access_duplicate_review_assert($exitCode === 0, "{$label} process worker should exit cleanly: {$stdout}{$stderr}");
}

if ((string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_WORKER') === '1') {
    videochat_call_access_duplicate_review_parallel_worker(
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_DATABASE'),
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_ACCESS_ID'),
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_START'),
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_RESULT'),
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_SESSION_ID'),
        (int) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_USER_ID'),
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_AUTH_SESSION_ID'),
        (string) getenv('VIDEOCHAT_CALL_ACCESS_DUPLICATE_REVIEW_LABEL')
    );
}
...
try {
    ...
    $loginSwitchFlags = $pdo->query("SELECT * FROM call_access_review_flags WHERE reason = 'duplicate_personalized_link'")->fetchAll();
    videochat_call_access_duplicate_review_assert(count($loginSwitchFlags) === 1, 'login switch should create one duplicate review flag');
    ...
    $flagRows = $pdo->query("SELECT * FROM call_access_review_flags WHERE reason = 'duplicate_personalized_link'")->fetchAll();
    ...
    $wrongHostOne = $callAccessRoute(
        '/session',
        'POST',
        ['Authorization' => 'Bearer sess_duplicate_second', 'Content-Type' => 'application/json'],
    ...
    $reviewFlagCount = (int) $pdo->query("SELECT COUNT(*) FROM call_access_review_flags WHERE reason = 'duplicate_personalized_link'")->fetchColumn();
...
SQL
    ...
    $loginSwitchAuditPayload = json_decode((string) $loginSwitchAudit->fetchColumn(), true);
    ...
    if (function_exists('pcntl_fork') || function_exists('proc_open')) {
        ...
        if (function_exists('pcntl_fork')) {
            ...
            $targetWorker = videochat_call_access_duplicate_review_spawn_process_worker(
                $databasePath,
                $accessId,
                $parallelStartPath,
                $parallelTargetPath,
            ...
            $thirdWorker = videochat_call_access_duplicate_review_spawn_process_worker(
                $databasePath,
                $accessId,
                $parallelStartPath,
                $parallelThirdPath,
        ...
        if (function_exists('pcntl_fork')) {
            $targetStatus = 0;
            $thirdStatus = 0;
            pcntl_waitpid((int) $targetPid, $targetStatus);
            pcntl_waitpid((int) $thirdPid, $thirdStatus);
            ...
            videochat_call_access_duplicate_review_wait_process_worker(is_array($targetWorker) ? $targetWorker : [], 'target');
            videochat_call_access_duplicate_review_wait_process_worker(is_array($thirdWorker) ? $thirdWorker : [], 'third-account');
        }
        ...
        $parallelPdo = videochat_open_sqlite_pdo($databasePath);
        $parallelSessionCount = (int) $parallelPdo->query("SELECT COUNT(*) FROM sessions WHERE id = 'sess_duplicate_target_parallel_access' AND user_id = " . $targetUserId)->fetchColumn();
        ...
        $foreignParallelSessionCount = (int) $parallelPdo->query("SELECT COUNT(*) FROM sessions WHERE id = 'sess_duplicate_third_should_not_issue_parallel' OR (user_id = " . $thirdUserId . " AND id LIKE 'sess_duplicate_third_should_not_issue%')")->fetchColumn();
        ...
        $freshParallelAccess = videochat_fetch_call_access_link($parallelPdo, $accessId, $defaultTenantId);
        ...
        $thirdFlagQuery = $parallelPdo->prepare(
            <<<'SQL'
SELECT *
...
SQL
        ...
        $parallelAudit = $parallelPdo->prepare(
            <<<'SQL'
SELECT COUNT(*)
FROM videochat_audit_events
WHERE event_type = 'call_access_duplicate_personalized_link_review'
...
SQL
        ...
        fwrite(STDOUT, "[call-access-duplicate-review-contract] SKIP: no pcntl_fork/proc_open available for parallel race subcheck\n");
    }
