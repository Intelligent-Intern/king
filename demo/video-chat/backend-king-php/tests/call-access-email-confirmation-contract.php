...
SQL
    ...
    $insertExpired->execute([
        ...
        ':token_fingerprint' => videochat_call_access_account_confirmation_token_fingerprint($expiredToken),
    ...
    $confirmationRows = $pdo->query('SELECT id, token_fingerprint, pending_payload_json, recipient_email_fingerprint, access_fingerprint FROM call_access_account_update_confirmations')->fetchAll();
    $confirmationDump = json_encode($confirmationRows, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?: '';
    videochat_call_access_email_confirmation_assert_no_needles($confirmationDump, [$firstToken, $accessId, $linkEmail, $linkName, $hostEmail, $hostName, 'sess_confirmation_current'], 'confirmation storage');
    ...
    videochat_call_access_email_confirmation_assert($auditFailed >= 2, 'failed confirmation attempts should be audit-logged');
