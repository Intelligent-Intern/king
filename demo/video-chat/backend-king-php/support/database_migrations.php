...
SQL,
        ...
        58 => ['name' => '0058_call_stt_settings', 'statements' => ["CREATE TABLE IF NOT EXISTS call_stt_settings (call_id TEXT PRIMARY KEY REFERENCES calls(id) ON UPDATE CASCADE ON DELETE CASCADE, enabled INTEGER NOT NULL DEFAULT 0 CHECK (enabled IN (0, 1)), updated_by_user_id INTEGER REFERENCES users(id) ON UPDATE CASCADE ON DELETE SET NULL, updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')), created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')))", 'CREATE INDEX IF NOT EXISTS idx_call_stt_settings_enabled ON call_stt_settings(enabled)']],
        59 => ['name' => '0059_user_superadmin_flag', 'statements' => ['ALTER TABLE users ADD COLUMN is_superadmin INTEGER NOT NULL DEFAULT 0 CHECK (is_superadmin IN (0, 1))', 'CREATE INDEX IF NOT EXISTS idx_users_superadmin ON users(is_superadmin, status)', "UPDATE users SET is_superadmin = 1 WHERE id = 1 AND lower(email) = 'admin@intelligent-intern.com' AND role_id = (SELECT id FROM roles WHERE slug = 'admin' LIMIT 1)"]],
