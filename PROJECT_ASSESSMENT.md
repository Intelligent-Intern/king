...
- `./scripts/test-extension.sh`: `361/361` passing
...
- `extension/tests`: `361` PHPT files
...
- real HTTP/1, HTTP/2, and HTTP/3 client request paths, including reuse, repeated pooled HTTP/2 mixed-load bursts, streaming, bodiless-response handling, explicit HTTP/1 and HTTP/2 abort/reset failure mapping, explicit HTTP/3 handshake-failure and transport-close mapping, and cancel/timeout contracts
...
- The next missing proof is concrete and repo-local: real listener coverage for server HTTP/2 and HTTP/3, HTTP/3 connection reuse/ticket behavior, and more explicit WebSocket violation/abort handling.
