...
surfaces inside the local tree. They are now concentrated in six narrower
areas:

- deeper transport and listener failure-path and load-bound verification across HTTP/1, HTTP/3, WebSocket, and server drain/normalization paths
- stronger backup/export/restore snapshot consistency plus post-restore and post-import integrity guarantees
- broader object-store provider-quota classification and remaining cross-backend failure characterization around the now-real core/cloud surface
- orchestrator continuation after host restart plus broader multi-worker and multi-host control-plane depth
...
- longer-haul telemetry/autoscaling characterization and systematic full-surface security closure
...
- `./infra/scripts/test-extension.sh`: `443/443` passing
...
- `extension/tests`: `443` PHPT files
...
### Security Closure Depth

- Recent hardening closed the currently known mainline findings in HTTP/2 one-shot request-body bounds, HTTP/3 one-shot completion, MCP transfer-key truncation, MCP peer targeting, object-store cloud metadata header emission, object-store metadata-cache growth, Semantic DNS live-probe SSRF, object-store local/distributed TOCTOU read paths, and privileged workflow-run source materialization.
- The remaining work is no longer about those obvious exposed-path regressions on `main`; it is the broader full-surface security review, secret-handling discipline, negative-input depth, and release-gated closure tracked in `READYNESS_TRACKER.md`.
