...
- broader Smart-DNS distributed-topology validation beyond the current on-wire listener proof, local query failure/recovery, concurrent-write, live-signal, split-brain/partial-failure, and concurrent mother-node churn proof
...
- `./infra/scripts/test-extension.sh`: `495/495` passing
...
- `extension/tests`: `495` PHPT files
...
- The remaining Smart-DNS work is real distributed topology validation beyond the current on-wire listener and local persisted-state slice rather than more local config cleanup, bounded local failure/recovery basics, restart-state basics, or local concurrent-write/mother-node churn correctness.
