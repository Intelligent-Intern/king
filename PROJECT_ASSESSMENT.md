...
- `./scripts/test-extension.sh`: `322/322` passing
...
- `extension/tests`: `322` PHPT files
...
- MCP request/upload/download parity against a real remote peer with propagated timeout, deadline, cancellation controls, 1 MiB payload roundtrips, and same-host partial-failure / peer-restart recovery coverage
...
- MCP now propagates timeout, deadline, and cancellation controls through the real peer protocol, and same-host large-payload plus partial-failure / peer-restart recovery are verified, but bounded concurrency, parallel-transfer backpressure, deeper restart semantics, and multi-host recovery depth are still open.
