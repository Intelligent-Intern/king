...
- broader Smart-DNS distributed-topology validation beyond the current on-wire listener proof, stale-peer rejoin healing after partial durable-state loss, local query failure/recovery, concurrent-write, live-signal, split-brain/partial-failure, and concurrent mother-node churn proof
...
- `./infra/scripts/test-extension.sh`: `496/496` passing
...
- `extension/tests`: `496` PHPT files
...
- The remaining Smart-DNS work is real distributed topology validation beyond the current on-wire listener, restart-safe persisted-state slice, and stale-peer partial-loss healing proof rather than more local config cleanup, bounded local failure/recovery basics, restart-state basics, or local concurrent-write/mother-node churn correctness.
