...
- `./scripts/test-extension.sh`: `324/324` passing
...
- `extension/tests`: `324` PHPT files
...
- MCP request/upload/download parity against a real remote peer with propagated timeout, deadline, cancellation controls, 1 MiB payload roundtrips, parallel-transfer backpressure isolation, explicit single-flight reentry guards, and same-host partial-failure / peer-restart recovery coverage
