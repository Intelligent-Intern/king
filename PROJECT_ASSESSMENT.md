...
The active explicitly requested `20`-issue repo-local batch is currently in
flight through `ISSUES.md`. Verified leaves roll into this file as they close.
The next `20` executable leaves are only pulled into `ISSUES.md` when the
active batch is exhausted and a new one is explicitly requested.
...
- `./infra/scripts/test-extension.sh`: `515/515` passing
...
- `extension/tests`: `515` PHPT files
