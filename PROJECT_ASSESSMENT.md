...
- `./scripts/test-extension.sh`: `320/320` passing
...
- `extension/tests`: `320` PHPT files
...
- MCP request/upload/download parity against a real remote peer with propagated timeout, deadline, cancellation controls, and targeted remote failure coverage
...
- MCP now propagates timeout, deadline, and cancellation controls through the real peer protocol, but larger-payload, bounded-concurrency, restart, and multi-host recovery depth are still open.
