...
- `./infra/scripts/test-extension.sh`: `387/387` passing
...
- `extension/tests`: `387` PHPT files
...
- object-store local filesystem persistence plus real `cloud_s3` payload transport with explicit `local_fs+cloud_s3_sidecars` runtime/system contract, `memory_cache -> local_fs` compatibility aliasing, `.meta` sidecars as the current metadata reference across the verified backends, explicit `cloud_s3` credential-rejection and endpoint-connect-failure detection through runtime adapter status/error surfaces, simulated `distributed`/`cloud_gcs`/`cloud_azure` fencing, CDN cache/runtime behavior, and confined backup/restore/import/export paths
