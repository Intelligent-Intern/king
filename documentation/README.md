...
| [Flow PHP and ETL on King](./flow-php-etl.md) | The contract between King runtime services and userland dataflow or ETL layers, including current repo-local source, sink, object-store dataset bridge, serialization/schema bridge, checkpoint, execution-backend, control-plane, failure-taxonomy, partitioning/backpressure, SQL/pgvector MCP-bridge, MCP service-discovery/failover helpers, OO object-store ingest guidance for originals/artifacts/streaming/viewer delivery, and one canonical multi-service RAG orchestration reference flow (`chat + ingest + parse + embed + retrieve + worker lifecycle`). | Read when building ETL, batch movement, or dataflow pipelines on top of King. |
