...
It reports any unavailable static contract by name. It then runs the
SQLite-backed backend IAM contracts only when the current `PHP_BIN` loads
`pdo_sqlite`. If `pdo_sqlite` is missing, the command prints a `BLOCKED` list
for those backend proofs and exits successfully; that output is not a backend
SQLite proof.
