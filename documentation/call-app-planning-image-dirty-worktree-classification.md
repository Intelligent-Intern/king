...
| --- | --- | --- |
| Generic package asset names, `public/<app-key>.css` and `public/<app-key>.js` | Already preserved in `demo/call-apps/README.md` and the package layout contract. | Keep integrated wording. |
| Legacy package root `demo/call-apps/image-planning/` and app key `image-planning` | Superseded by `demo/call-apps/planning-image/` and app key `planning-image`. | Do not copy the legacy package into integration. |
...
files from `demo/call-apps/image-planning/` should be copied into integration.
