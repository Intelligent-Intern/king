...
| `local/iam-e2e-foreign-personalized-mismatch` | `17618082` | Adds wrong-host and correct-host foreign personalized-link journeys with verified current-session context and no foreign target data. | At IAM5-08 extraction time, current strong-mismatch privacy contracts preserved the denied no-leak behavior and correct-host account-update journeys stayed outside that lane. IAM11-08 now covers the source-equivalent correct-host decline/update-confirm-email flow through the focused JoinView extraction, strong-mismatch helper, browser E2E, and contract gate. |
