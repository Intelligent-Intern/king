...
- Recent QUIC bootstrap closure: the build path now rehydrates a pinned `lsquic` commit, pinned BoringSSL submodule commit, tracked workspace lockfile, and pinned `wirefilter` revision without branch-based fallbacks or unlocked cargo retries.
...
- [x] Fully pin the `lsquic` dependency
