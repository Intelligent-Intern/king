...
- Recent QUIC bootstrap closure: the build path now rehydrates pinned LSQUIC, BoringSSL, ls-qpack, and ls-hpack source archives from `infra/scripts/lsquic-bootstrap.lock` without branch-based fallbacks, legacy Quiche bootstrap inputs, or unlocked Cargo retries.
...
- Recent video-chat call-create closure: `POST /api/calls` now has explicit endpoint-contract proof for typed request-body and field validation failures, normalized create envelopes with internal/external participant totals, persisted call+participant rows, and fail-closed no-partial-persist behavior on rejected participant payloads, proven by `backend-king-php/tests/call-create-endpoint-contract.sh`.
- Recent video-chat call-update closure: `PATCH /api/calls/{id}` now has explicit endpoint-contract proof for participant-diff updates, typed validation/forbidden/not-found envelopes, immutable-status rejection, and explicit no-global-resend invite dispatch flags (`global_resend_triggered=false`, `explicit_action_required=true`), proven by `backend-king-php/tests/call-update-endpoint-contract.sh`.
- Recent video-chat call-cancel closure: `POST /api/calls/{id}/cancel` now has explicit endpoint-contract proof for typed request-body and payload validation failures, forbidden/not-found/state-conflict envelopes, persisted cancellation reason/message/timestamp semantics, participant invite-state cancellation propagation, and active-join exclusion from user list scope, proven by `backend-king-php/tests/call-cancel-endpoint-contract.sh`.
- Recent video-chat invite-create closure: `POST /api/invite-codes` now has explicit endpoint-contract proof for invalid-body and validation failures, owner/admin/moderator call-scope authorization boundaries, deterministic UUID-backed code/id creation, server-managed scope TTL policy (`call`/`room`), and persisted invite row shape guarantees, proven by `backend-king-php/tests/invite-code-create-endpoint-contract.sh`.
...
- Recent video-chat invite-copy closure: `POST /api/invite-codes` now returns preview-only metadata without raw invite secrets, while `POST /api/invite-codes/{id}/copy` exposes the raw code only through an explicit copy boundary with issuer/admin/call-editor authorization, expiry handling, catalog/runtime registration, and protected-API matrix coverage, proven by `backend-king-php/tests/invite-code-copy-boundary-contract.sh`.
...
- Recent video-chat room-presence closure: websocket room membership is now server-authoritative with deterministic `room/snapshot` plus `room/joined`/`room/left` delta behavior across join, leave, room-change, and reconnect/resync paths, with strengthened phantom-user and reconnect contract proof in `backend-king-php/tests/realtime-presence-contract.sh`.
...
- Recent video-chat realtime-reaction closure: room reactions now enforce deterministic emoji/client-id payload boundaries and per-user server-side throttle windows with explicit `retry_after_ms` diagnostics while preserving room-scoped stream responsiveness under burst pressure, proven by `backend-king-php/tests/realtime-reaction-contract.sh` and `backend-king-php/tests/contract-catalog-parity-contract.sh`.
...
- [x] Establish a UI-parity acceptance matrix tied to executable checks instead of prose-only claims.
...
- [x] Add API boundaries for invite-preview/copy flows so invite code is not leaked as table-column default data.
...
- [x] Fully pin the LSQUIC dependency set
...
- [x] Validate a real end-to-end ETL/dataflow pipeline on top of King runtime services under local and remote-worker execution
...
- [x] Define username/password authentication with hashed credentials for the model-inference demo — proven: `auth-store-contract` (72 rules) + `auth-seed-contract` (44 rules) verify bcrypt round-trip, envelope hygiene (no password_hash leaks), and idempotent demo-user fixtures (admin/alice/bob) with env overrides (#A-1, #A-6)
- [x] Define opaque bearer-token session layer with TTL + revocation — proven: `auth-endpoint-contract` (45 rules) verifies POST /api/auth/login issue, POST /api/auth/logout revoke (idempotent), GET /api/auth/whoami reads; `auth-middleware-contract` (25 rules) verifies Bearer extraction, case-insensitive header, revoked/expired rejection, non-blocking anonymous fallthrough (#A-2, #A-3)
- [x] Define conversation ownership binding to authenticated user — proven: `conversation-ownership-contract` (28 rules) verifies `conversations.user_ref` is populated on append-turn when authenticated, NULL when anonymous, cross-user append does NOT reassign ownership; `GET /api/conversations/{id}/messages` + `DELETE` enforce 403 `ownership_denied`; `GET /api/conversations/me` returns only the caller's owned sessions (#A-4)
- [x] Define WebSocket handshake-time auth check — proven: `realtime-auth-contract` (13 rules) verifies the WS upgrade extracts Bearer from `Authorization` header AND `?auth_token=<32-hex>` query fallback (browsers can't set custom WS headers), hydrates request.user for the stream session, and honestly fences per-frame revalidation as out-of-scope (#A-5)
