...
- Recent userland orchestrator contract closure: the public docs and stub surface now explicitly separate durable tool definitions from executable userland handlers, treat closures/object graphs/resources/controller memory as non-durable execution state, pin the public execution contract to per-process handler registration by tool name across local, file-worker, and remote-peer boundaries, and now back the local plus file-worker slices with a real handler API, direct local execution and restart/resume proof, an explicit local handler input/output contract, a queued-run durable `handler_boundary`, pre-claim/pre-resume worker readiness validation, and real file-worker execution plus worker-restart continuation; the remaining open work is remote-peer execution depth plus the broader failure/control/status slices below.
