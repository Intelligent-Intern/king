...
- Recent userland orchestrator contract closure: the public docs and stub surface now explicitly separate durable tool definitions from executable userland handlers, treat closures/object graphs/resources/controller memory as non-durable execution state, pin the public execution contract to per-process handler registration by tool name across local, file-worker, and remote-peer boundaries, and now back that local slice with a real handler API, direct local execution and restart/resume proof, an explicit local handler input/output contract, a queued-run durable `handler_boundary`, and pre-claim/pre-resume worker readiness validation; the remaining open work is file-worker and remote-peer execution depth.
