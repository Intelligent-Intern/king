--TEST--
King object-store incremental restore fails closed when the snapshot archive is partially corrupted
--INI--
king.security_allow_config_override=1
--FILE--
<?php

function king_object_store_469_remove_tree(string $path): void
{
    if (!is_dir($path)) {
        return;
    }
    ...
    foreach (scandir($path) as $entry) {
        ...
        $child = $path . '/' . $entry;
        if (is_dir($child)) {
            king_object_store_469_remove_tree($child);
            @rmdir($child);
            continue;
        }
...
$source = sys_get_temp_dir() . '/king_object_store_restore_corrupt_source_469_' . getmypid();
$target = sys_get_temp_dir() . '/king_object_store_restore_corrupt_target_469_' . getmypid();
...
foreach ([$source, $target] as $path) {
    king_object_store_469_remove_tree($path);
...
@unlink($importIncrementalSnapshot . '/asset-3.meta');
