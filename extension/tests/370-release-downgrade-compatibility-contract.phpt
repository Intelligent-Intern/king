...
$script = $rootDir . '/infra/scripts/check-release-downgrade.sh';
...
var_dump($status === 0);
var_dump(str_contains($help, 'Usage: ./infra/scripts/check-release-downgrade.sh --from-ref REF'));
...
var_dump(str_contains($ci, '../infra/scripts/check-release-downgrade.sh --from-ref'));
