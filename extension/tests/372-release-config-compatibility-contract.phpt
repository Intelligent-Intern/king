...
$script = $extensionDir . '/scripts/check-config-compatibility-matrix.sh';
$fixture = $extensionDir . '/scripts/runtime-config-compatibility.php';
...
var_dump($status === 0);
var_dump(str_contains($help, 'Usage: ./scripts/check-config-compatibility-matrix.sh [--archive PATH] [--php-bin BIN] [--artifacts-dir DIR]'));
...
var_dump(str_contains($ci, './scripts/check-config-compatibility-matrix.sh'));
