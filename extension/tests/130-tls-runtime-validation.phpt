...
var_dump(king_client_tls_import_session_ticket($openSession, str_repeat('a', 513)));
var_dump(king_get_last_error());
?>
--EXPECTF--
bool(false)
...
bool(false)
string(%d) "king_import_session_ticket() ticket length must be between 1 and 512 bytes."
bool(true)
bool(false)
string(%d) "king_client_tls_import_session_ticket() cannot import a ticket into a closed session."
bool(false)
string(%d) "king_client_tls_import_session_ticket() ticket length must be between 1 and 512 bytes."
