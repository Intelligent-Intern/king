--TEST--
King object-store cloud_s3 surfaces endpoint throttling through runtime status and write failures
--SKIPIF--
<?php
...
$root = sys_get_temp_dir() . '/king_object_store_s3_throttle_' . getmypid();
if (!is_dir($root)) {
...
$mock = king_object_store_s3_mock_start_server(
    ...
    [
        'forced_responses' => [
            [
                'method' => 'GET',
                'target' => '/throttle-test?list-type=2',
                'status' => 429,
...
try {
    ...
    var_dump(str_contains($e->getMessage(), "cloud_s3 write for 'doc-s3' was throttled"));
...
var_dump(in_array('GET /throttle-test?list-type=2', $targets, true));
var_dump(in_array('HEAD /throttle-test/doc-s3', $targets, true));
var_dump(in_array('PUT /throttle-test/doc-s3', $targets, true));
var_dump(count(array_filter(
    $capture['events'],
    static fn(array $event): bool => ($event['forced_status'] ?? 0) === 429 && ($event['forced_error_code'] ?? '') === 'TooManyRequestsException'
)) >= 2);
var_dump(count(array_filter(
    $capture['events'],
    static fn(array $event): bool => ($event['forced_status'] ?? 0) === 503 && ($event['forced_error_code'] ?? '') === 'SlowDown'
)) >= 1);
