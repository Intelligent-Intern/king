--TEST--
King HTTP/1 response_stream follows absolute redirects before exposing the final response
--FILE--
<?php
function king_http1_start_streaming_absolute_redirect_server(): array
{
    ...
    $script = tempnam(sys_get_temp_dir(), 'king-http1-stream-redirect-absolute-');
    $release = tempnam(sys_get_temp_dir(), 'king-http1-stream-redirect-release-');
    @unlink($release);
...
PHP);
    ...
    if (!is_resource($process)) {
        @unlink($script);
        throw new RuntimeException('failed to launch local HTTP/1 streaming absolute redirect server');
    }
    ...
    if ($ready !== "READY\n") {
        ...
        @unlink($script);
        @unlink($release);
        throw new RuntimeException('local HTTP/1 streaming absolute redirect server failed: ' . trim($stderr));
    }

    return [$process, $pipes, $script, $release, (int) $port];
}

function king_http1_stop_streaming_absolute_redirect_server(array $server): void
{
...
$server = king_http1_start_streaming_absolute_redirect_server();
try {
    $context = king_http1_request_send(
        'http://127.0.0.1:' . $server[4] . '/start',
        'GET',
        null,
        null,
        [
    ...
    king_http1_stop_streaming_absolute_redirect_server($server);
}
?>
--EXPECT--
