--TEST--
King HTTP/1 chunked response_stream returns a drained keep-alive socket to the reuse pool
--FILE--
<?php
function king_http1_start_chunked_reuse_after_drain_server(): array
{
    ...
    $script = tempnam(sys_get_temp_dir(), 'king-http1-chunked-reuse-after-drain-');
...
PHP);
    ...
    if (!is_resource($process)) {
        @unlink($script);
        throw new RuntimeException('failed to launch local HTTP/1 chunked reuse-after-drain server');
    }
    ...
    if ($ready !== "READY\n") {
        ...
        proc_close($process);
        @unlink($script);
        throw new RuntimeException('local HTTP/1 chunked reuse-after-drain server failed: ' . trim($stderr));
    }

    return [$process, $pipes, $script, (int) $port];
}

function king_http1_stop_chunked_reuse_after_drain_server(array $server): void
{
...
$server = king_http1_start_chunked_reuse_after_drain_server();
try {
    $context = king_http1_request_send(
        'http://127.0.0.1:' . $server[3] . '/chunked-reuse',
        'GET',
        null,
        null,
        [
    ...
    king_http1_stop_chunked_reuse_after_drain_server($server);
}
?>
--EXPECT--
string(5) "alpha"
