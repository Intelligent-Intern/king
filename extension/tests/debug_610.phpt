...
PHP;
file_put_contents($controllerScript, str_replace('__EXECUTION_BACKEND_PATH__', var_export($executionBackendPath, true), $controllerTemplate));
...
$observerCommand = static function (string $runId) use ($baseCommand, $observerScript): string {
    return sprintf('%s %s', sprintf($baseCommand, escapeshellarg($observerScript)), escapeshellarg($runId));
};

$resumeCommand = static function (string $runId) use ($baseCommand, $resumeScript): string {
    return sprintf('%s %s', sprintf($baseCommand, escapeshellarg($resumeScript)), escapeshellarg($runId));
};

echo "Controller script: $controllerScript\n";
$controllerArgv = [
...
for ($i = 0; $i < 400; $i++) {
    ...
    if ($observerStatus === 0 && is_array($snapshot) && ($snapshot['run_id'] ?? null) === $runId && ($snapshot['status'] ?? null) === 'running') {
        $runningObserved = true;
    }
    if ($runningObserved) break;
    usleep(10000);
}
