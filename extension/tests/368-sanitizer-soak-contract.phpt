...
$script = $extensionDir . '/scripts/soak-runtime.sh';
...
var_dump($status === 0);
var_dump(str_contains($help, 'Usage: ./scripts/soak-runtime.sh <asan|ubsan|leak>'));
...
var_dump(str_contains($ci, './scripts/soak-runtime.sh "${{ matrix.mode }}"'));
