...
zend_result king_iibin_decode_batch_varint(
    ...
    if (!king_proto_registry_has_schema(schema_name)) {
        king_throw_proto_schema_not_defined(schema_name, "batch decode (varint)");
        return FAILURE;
    }

    runtime_schema = king_proto_registry_get_runtime_schema(schema_name);
    if (!runtime_schema) {
        king_throw_proto_schema_registered_but_unavailable(schema_name, "batch decode (varint)");
        return FAILURE;
    }
    ...
    if (Z_TYPE_P(binary_records) != IS_STRING) {
        king_iibin_decode_mode_destroy(&decode_mode);
        zend_throw_exception(king_ce_validation_exception,
            "decodeBatchVarint expects a string (concatenated batch)", 0);
        return FAILURE;
    }
    ...
    if (total_len > 0) {
        ...
        while (p < data_end) {
            ...
            ZVAL_UNDEF(&decoded);
            if (!king_proto_runtime_decode_schema_payload(
                    schema_name, runtime_schema, p, (size_t)rec_len,
                    false, &decoded)) {
                break;
            }
            p += rec_len;
