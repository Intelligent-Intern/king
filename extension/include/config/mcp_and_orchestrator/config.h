 ...
 *
 * PURPOSE:
 * Declares the MCP/orchestrator config apply helpers for both the live
 * module-global runtime state and `King\Config` snapshot targets.
 *
 * ARCHITECTURE:
 * MCP/orchestrator participates directly in the central config-snapshot
 * override pipeline through
 * `kg_config_mcp_and_orchestrator_apply_userland_config_to()`. The plain
 * `kg_config_mcp_and_orchestrator_apply_userland_config()` variant remains
 * the policy-gated helper for writing into the live module-global state.
 * =========================================================================
 */
...
/**
 * @brief Applies MCP/orchestrator settings from a PHP array to a target config struct.
 * @param target The target MCP/orchestrator config snapshot to mutate.
 * @param config_arr A zval pointer to a PHP array containing the key-value
 * pairs of the configuration to apply.
 * @return `SUCCESS` if all settings were successfully validated and applied,
 * `FAILURE` otherwise.
