...
struct llama_cparams {
    ...
    int32_t layer_start;  // first layer to compute (0 = first layer)
    int32_t layer_end;    // last layer to compute (-1 = all layers)
};
...
struct llm_build_context {
    ...
    const int32_t layer_start;  // first layer to compute (0 = first layer)
    const int32_t layer_end;    // last layer to compute (-1 = all layers)
...
n_ctx_orig       (cparams.n_ctx_orig_yarn),
        ...
        layer_start    (cparams.layer_start),
        layer_end      (cparams.layer_end >= 0 ? cparams.layer_end : hparams.n_layer - 1) {
...
// KQ_mask (mask for 1 head, it will be broadcasted to all heads)
    ...
    struct ggml_cgraph * build_t5() {
        ...
        if (lctx.is_encoding) {
            ...
            struct ggml_tensor * KQ_mask_enc = build_inp_KQ_mask();
