...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "-ls" || arg == "--layer-start") {
        CHECK_ARG
        params.layer_start = std::stoi(argv[i]);
        return true;
    }
    if (arg == "-le" || arg == "--layer-end") {
        CHECK_ARG
        params.layer_end = std::stoi(argv[i]);
        return true;
    }
    if (arg == "--kv-cache-out") {
        CHECK_ARG
        params.kv_cache_out = argv[i];
        return true;
    }
    if (arg == "--kv-cache-in") {
        CHECK_ARG
...
void gpt_params_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    options.push_back({ "*",           "       --kv-cache-out FNAME",     "save KV cache state to file after generation" });
    options.push_back({ "*",           "       --kv-cache-in FNAME",     "load KV cache state from file before generation" });
