...
- `King\Inference`, `King\Inference\Model`, `King\Inference\Stream`, and the
  ...
  of requiring callers to provide `graph` or `graphs` manually.
  ...
  device vector operations, RMSNorm, RoPE, attention score, attention softmax,
  attention value aggregation, FFN/SwiGLU, final output projection paths, and
  bounded top-K logits readback as the first decoder compute leaves. The GPU
  ...
  GPU metadata exposes that missing bridge explicitly: device KV cache, the
  decoder graph executor, and the prompt decoder loop.
