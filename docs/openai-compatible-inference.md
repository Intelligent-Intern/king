...
and `ffn_down_tensor_pattern` must be non-empty strings when provided.
...
when provided. King extracts text from `text`, `content`, or `refusal` fields
and feeds that into the local text-generation prompt. Non-text parts such as
image, audio, or file payloads are rejected for this local text inference route
...
`graphs`. `graph` must be a JSON object, `graphs` must be a JSON array, and
`graph_options` must be a JSON object where provided. Native graph requests
still return OpenAI-compatible Chat Completions chunks or JSON responses, but
the router does not synthesize a
