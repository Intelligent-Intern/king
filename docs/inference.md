...
needs the remaining RoPE, KV attention, residual, FFN, and logits device ops
plus bounded logits results before the prompt loop can emit decoded tokens.
...
`decoder_graph_slice_execution_ready`, `decoder_prompt_loop_ready`,
...
that linear output, the executor runs the CUDA vector slice kernel and then
releases the temporary output buffers. Temporary buffers are released after the
