...
remains blocked until King finishes the final norm and bounded logits path to
the sampler.
...
needs the remaining final norm and logits device ops plus bounded logits
...
`gpu_decoder_graph_ffn_output_residual_execution`, `gpu_prompt_decoder_loop`,
...
the admitted initial device chain. It keeps
`device_execution_result_ready=false` until the CUDA executor carries the
remaining final norm and logits ops through bounded logits and writes real
